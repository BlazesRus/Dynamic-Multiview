// DynViewDlg.cpp : implementation file
//

#include "StdMultiView.h"
#include "DynViewsApp.h"
#include "DynViewDlg.h"
...
DynViewDlg::DynViewDlg(CWnd* pParent /*=NULL*/)
	: CDialog(DynViewDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(DynViewDlg)
...
void DynViewDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(DynViewDlg)
	DDX_Control(pDX, IDC_TABCTRL, m_tabCtrl);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(DynViewDlg, CDialog)
	//{{AFX_MSG_MAP(DynViewDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
/////////////////////////////////////////////////////////////////////////////
// DynViewDlg message handlers

BOOL DynViewDlg::OnInitDialog()
{
...
void DynViewDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR DynViewDlg::OnQueryDragIcon()
{
 ...
 *
 */
void DynViewDlg::SelectTab(int nTab)
{
	DynView* pViewSelected = NULL;
	POSITION pos = m_listViews.GetHeadPosition();

	// Hide all views under the tabControl
	while(pos != NULL) {
		DynView* pView = (DynView*) m_listViews.GetNext(pos);
...
void DynViewDlg::OnSelchangeTabctrl(NMHDR* pNMHDR, LRESULT* pResult) 
{	
...
void DynViewDlg::OnAddview() 
{
	AddView(VIEW_2D);
}

// Handle for button click "Add 3D/OpenGL View"
void DynViewDlg::OnAddOpenGlView() 
{
	AddView(VIEW_OPENGL);
}
 ...
 */
void DynViewDlg::AddView(VIEW_TYPE viewType)
{
	// ID of the new view
